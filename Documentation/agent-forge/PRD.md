...
| **Unexpected / unparsable model output** | Schema validation fails (NFR-CONTRACT-1) | Reject the output; one repair attempt; else deterministic fallback | Core data without the malformed section | Validation error + raw output ref | NFR-CONTRACT-1, NFR-REL-1 |
