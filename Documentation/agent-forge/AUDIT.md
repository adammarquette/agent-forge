...
**Date:** 2026-07-07 (system audit) · 2026-07-09 (EHR-launch dependency review, §8)
...
3. **Custom module** (`interface/modules/custom_modules/`) — first-class extension mechanism; the co-pilot UI can ship as a module (`oe-module-*` precedent exists; see §8 for the concrete build).
...
- **Limitations:** this audit reflects the base system pre-agent. It does not yet assess the agent's own code beyond §8. Some legacy `/library` and `/interface` query paths were sampled, not exhaustively reviewed; anything the agent integrates with should be re-reviewed at integration time.

---

## 8. EHR-Launch Dependency Review (2026-07-09)

*(A later, narrower review of the launch-integration slice specifically — the implementation
following from §4.4's module recommendation. Findings here are tracked as GitLab issues
`agent-forge#1` (AF-1), `agent-forge#2` (AF-2), `agent-forge-copilot#34` (CP-1), each `Blocked by`
the previous.)*

### 8.1 Missing OpenEMR-side launch implementation (AF-1)
ARCHITECTURE.md §2/§3.1 assert an EHR-initiated SMART launch, but as of the prior audit pass no
custom module existed to originate it — the launch was self-initiated by the sidecar instead. **As
of this pass, a module scaffold now exists** (`oe-module-agentforge`, ARCHITECTURE.md §9.5): a
launch-URL builder and a `Bootstrap` class wired to OpenEMR's existing `RenderEvent` hook. It is not
yet installed on a running instance or wired end-to-end against a live chart render, so the gap is
narrowed but not closed.

### 8.2 Deployment gap (AF-2)
The Railway deployment target must run a build of this fork (module included), not a stock OpenEMR
image — otherwise the module, any seeded demo data (§5.2), and hardened config (§2) never reach the
runtime the sidecar tests against. `railway.json` / `docker/railway/Dockerfile` / `.gitlab-ci.yml`
already exist in this repo (`docs/DEPLOYMENT.md` has the setup runbook); what remains is validating
a deploy with the AF-1 module baked in, and confirming the dev-compose hardening gaps from §2 don't
carry into the deployed target.

### 8.3 Consumer-side dependency (CP-1)
The sidecar/BFF cannot switch from standalone launch to EHR-originated launch until AF-1 and AF-2
exist and are deployed. This keeps the OpenEMR-side work (this repo) the upstream blocker for the
sidecar integration work (`agent-forge-copilot`).

### 8.4 Recommended execution order
1. Finish wiring `oe-module-agentforge` into a real patient-chart render path and confirm it against
   a running instance (completes AF-1).
2. Deploy the fork to Railway with the module included and validate against the QA integration path
   — OAuth client + seeded FHIR patient still resolve (completes AF-2).
3. Update the sidecar/BFF to consume the module-originated launch (CP-1, tracked in the other repo).
